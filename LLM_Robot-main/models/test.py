...
STOP_SIGNAL_FILE = 'lidar_test/src/my_laser/scripts/stop_signal.txt'  # 中止信号文件
...
class SSHClient:
    ...
    def remove_file(self, file_path):
        """删除远程文件"""
        self.client.exec_command(f'rm -f {file_path}')
...
class Robot:
    ...
    def execute_code(self):
        ...
        output, errors = self.ssh_client.execute_command(f'python {REMOTE_PATH}')
        print("执行输出:", output)
        print("错误输出:", errors)
    ...
    def monitor_stop_signal(self, check_interval=5):
         """通过已建立的SSH连接持续监听远程是否发送中止信号，并返回中止状态和原因"""
         try:
            while True:
                  if self.ssh_client.check_file_exists(STOP_SIGNAL_FILE):
                     print("检测到中止信号，停止操作。")
                     # 读取中止原因（假设文件内容为原因）
                     stdin, stdout, stderr = self.ssh_client.client.exec_command(f'cat {STOP_SIGNAL_FILE}')
                     reason = stdout.read().decode().strip()
                     print(f"中止原因: {reason}")
                     # 删除中止信号文件
                     self.ssh_client.remove_file(STOP_SIGNAL_FILE)
                     return True, reason
                  print("未检测到中止信号，继续监听...")
...
if __name__ == "__main__":
    ...
    robot.send_code('print("Hello Robot")')
    robot.execute_code()  # 执行远程命令
    robot.monitor_stop_signal()  # 持续监听中止信号
    robot.get_log()  # 获取日志
    robot.shutdown()  # 关闭连接
